Feature: Adicionar categoria ao produto
    ...
    Eu quero <Adicionar uma categoria o produto>
...
Scenario: Categoria válida e produto válido apto a ter uma nova categoria adicionada (Padrão)
    ...
    Scenario: Categoria válida e produto válido inapto a ter uma nova categoria adicionada - quatidade máxima de categorias
    Dado (Given) [
        Um produto válido inapto a ter uma nova categoria adicionada - Ter (3) categorias já adicionadas
        Uma categoria válida
    ]
    Quando (When) [Solicitar a adição da categoria ao produto]
    Então (Then) [Um erro informando que o produto já possui número de categorias adicionadas]

     Scenario: Categoria válida e produto válido inapto a ter uma nova categoria adicionada - categoria já adicionada
    Dado (Given) [
        Um produto válido inapto a ter uma nova categoria adicionada - categorias já adicionada
        Uma categoria válida
    ]
    Quando (When) [Solicitar a adição da categoria ao produto]
